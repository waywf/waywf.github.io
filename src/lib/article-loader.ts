 ...
 * # 文章内容
 */
export async function loadArticleFromMarkdown(filePath: string): Promise<Article | null> {
  try {
 ...
 * 解析 Markdown 文章
 */
function parseMarkdownArticle(content: string, filePath: string): Article | null {
  // 提取 frontmatter
  ...
  // 提取文章 ID（从文件名）
  const id = filePath.split('/').pop()?.replace('.md', '') || Date.now().toString()
...
function parseFrontmatter(yaml: string): Record<string, any> {
  ...
  for (const line of lines) {
    ...
    if (cleanKey === 'tags') {
      result[cleanKey] = value.split(',').map(t => t.trim()).filter(Boolean)
    } else {
