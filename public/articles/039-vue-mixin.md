...
此篇主要手写 Vue2.0 源码-Mixin 混入原理上一篇咱们主要介绍了 Vue diff算法原理  核心是运用 diff算法来进行渲染优化 此篇主要包含 Mixin 混入 这是 Vue 里面非常关键的一个 api 在 Vue 初始化的时候起到了合并选项的重要作用
...
```javascript
// src/lifecycle.jsexport function callHook(vm, hook) {
  // 依次执行生命周期对应的方法
...
```javascript
// src/lifecycle.jsexport function mountComponent(vm, el) {
  vm.$el = el;
...
通过以上步骤 我们实现了 Vue 的 Mixin 混入原理 主要是利用 mergeOptions 方法把传入的选项混入到自己的 options 上面

生命周期的合并策略是把全部的生命周期都各自混入成了数组的形式依次调用

这样可以实现代码复用 提高开发效率

希望通过本文的介绍 大家能够理解 Vue 的 Mixin 混入原理 并在实际开发中灵活运用
